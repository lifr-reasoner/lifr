...
- *LiFR core algorithm update*: **Toggle automatic KB complements induction.** 
  - In order to maintain soundness and completeness within the DLP fragment, all concept complements and required related axioms are automatically added to the KB for every concept in the KB. I.e. for every concept A, the concept -A and the axiom (DISJOINT A -A) is inserted in the KB, which more than doubles the TBox size. In many real-world usage cases, it was empirically observed that complements are never asserted or inferred, therefore are redundant for the problems at question. To this end, the ability to toggle the complement induction will be offered, in order to reduced reasoning time and consumed resources to approximately less than half. 
